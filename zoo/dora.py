...
class model(BaseModel):
    def __init__(self, 
        ...
        # Base item embedding. In DoRA, this provides the base direction.
        self.embedding_p = nn.Embedding(num_embeddings=item_num, embedding_dim=embedding_dim)

        # DoRA magnitude parameter m.
        # Shape: [item_num, 1], so each item vector has its own learnable scale.
        self.dora_magnitude = nn.Embedding(num_embeddings=item_num, embedding_dim=1)
    ...
    def __init_weight(self, ):
        ...
        # Initialize magnitude from the current base embedding norm.
        self.sync_dora_magnitude()

    def sync_dora_magnitude(self):
        """
        Initialize / refresh DoRA magnitude from the base item embedding norm.

        Call this after replacing embedding_p.weight, e.g., after AE pretraining.
        """
        with torch.no_grad():
            mag = self.embedding_p.weight.norm(p=2, dim=1, keepdim=True).clamp_min(1e-6)
            self.dora_magnitude.weight.copy_(mag)

    def _dora_scale(self, item_id, direction):
        """Apply DoRA magnitude-direction decomposition to item vectors."""
        direction_norm = direction.norm(p=2, dim=-1, keepdim=True).clamp_min(1e-6)
        return self.dora_magnitude(item_id) * direction / direction_norm

    def emb_item(self, item_id):
        """
        DoRA item embedding:
            e_i = m_i * normalize(p_i + BA_i)

        where p_i is the pretrained/base item embedding and BA_i is the
        low-rank LoRA update produced by embedding_item.
        """
        base = self.embedding_p(item_id)
        lora_update = self.embedding_item(item_id)
        direction = base + lora_update
        return self._dora_scale(item_id, direction)

    def emb_item_c(self, item_id):
        """
        Compressed DoRA variant using only the low-rank adapter direction.
        This keeps compatibility with your original compressed=True branch.
        """
        direction = self.embedding_item(item_id)
        return self._dora_scale(item_id, direction)
...
class DoRA:
    ...
    def fit(self,):
        ...
        if not self.compressed:
            ...
            self.server.model.sync_dora_magnitude()
