...
class model(BaseModel):
    ...
    def set_b_trainable(self, trainable: bool):
        # B = embedding_item.linear
        self.embedding_item.linear.weight.requires_grad_(trainable)
...
class Client(ClientBase):
    ...
    def local_train(self, user, local_epoch, dataload, pre_train=False, compressed=False, update_b=False):
        ...
        # Warmup: B fixed. After warmup: periodic B update.
        if pre_train:
            self.model.set_b_trainable(False)
        else:
            self.model.set_b_trainable(update_b)
