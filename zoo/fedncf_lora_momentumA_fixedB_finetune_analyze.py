...
class FedNCF_Lora_MomentumA_FixedB_Finetune_Analyze:
    def __init__(self, 
    ...
    # ===== ADDED: helper functions for A/B convergence analysis =====
    ...
    def _log_AB_convergence_metrics(self, turn):
        ...
        if self.prev_track_delta_A is None:
            cos_delta_A = float("nan")
        else:
            cos_delta_A = self._safe_cosine(delta_A, self.prev_track_delta_A)

        if self.prev_track_delta_B is None:
            cos_delta_B = float("nan")
        else:
            cos_delta_B = self._safe_cosine(delta_B, self.prev_track_delta_B)

        # PyTorch Linear stores B as [embedding_dim, latent_dim],
        # so effective item low-rank embedding is A @ B.T.
    ...
    def fit(self):
        ...
        # ===== ADDED: initialize A/B tracking baseline before FL rounds =====
        ...
        for turn in range(self.train_turn):
            ...
            if (turn + 1) % 10 == 0:
                # ===== ADDED: log A/B convergence metrics every 10 rounds =====
