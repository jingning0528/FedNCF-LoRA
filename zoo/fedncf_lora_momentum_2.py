...
class model(BaseModel):
    ...
    def train_step_triple(self, users, pos, neg, global_model=None, freeze_B=False):
        self.train()
        # Always unfreeze A after Phase 1
        self.embedding_item.emb.weight.requires_grad_(True)
        if freeze_B:
            # Phase 3: freeze B, train A only
            self.embedding_item.linear.weight.requires_grad_(False)
        else:
            # Phase 2: B warmup, train A + B
            self.embedding_item.linear.weight.requires_grad_(True)
    ...
    def train_step_triple_pre(self, users, pos, neg, global_model=None):
        self.train()
        # Phase 1: AE warmup — only embedding_p is trained
        self.embedding_item.linear.weight.requires_grad_(False)
...
class Client(ClientBase):
    ...
    def upload_model(self, freeze_B=False):
        """
        Phase 1 & 2: upload everything (including B) for aggregation.
        Phase 3: only upload A (embedding_item.emb), NOT B (embedding_item.linear).
        """
        full_state = self.model.state_dict()
        if freeze_B:
            # Phase 3: exclude B from upload
            upload_state = {k: v.clone() for k, v in full_state.items()
                            if 'embedding_item.linear' not in k}
        else:
            # Phase 1 & 2: upload everything
            upload_state = {k: v.clone() for k, v in full_state.items()}
        return upload_state

    def local_train(self, user, local_epoch, dataload, pre_train=False, compressed=False, freeze_B=False):
        self.model.train()
        if self.task == "triple":
            ...
            for _ in range(local_epoch):
                if self.fedop == "fedprox":
                    if compressed:
                        ...
                        # Phase 2 (freeze_B=False) or Phase 3 (freeze_B=True)
                        loss = self.model.train_step_triple(users, pos, neg, self.global_model, freeze_B=freeze_B)
                else:
                    if compressed:
                        ...
                        # Phase 2 (freeze_B=False) or Phase 3 (freeze_B=True)
                        loss = self.model.train_step_triple(users, pos, neg, freeze_B=freeze_B)
        else:
...
class Server(ServerBase):
    ...
    def aggregation(self, user_list, model_list, num_list, loss_list, freeze_B=False, cdp=None, ldp=None):
        self.model.eval()
        ...
        for name in base_model_dict.keys():
            if 'embedding_item.linear' in name:
                if freeze_B:
                    # Phase 3: B is frozen globally — never updated
                    continue
                else:
                    # Phase 2: aggregate B normally via FedAvg
                ...
                # step 3: momentum update: v_A^t = beta * v_A^{t-1} + delta_A^t
...
class FedNCF_Lora_Momentum_2:
    def __init__(self, 
        ...
        self.ae_warmup_turns = 20                                      # Phase 1: fixed 20 rounds
        self.warmup_B_turns = int(kwargs.get("warmup_B_turns", 10))    # Phase 2: default 10 rounds
    ...
    def fit(self,):
        ...
        logging.info("Phase schedule: Phase1(AE warmup)=turns 0-{}, Phase2(B warmup)=turns {}-{}, Phase3(freeze B)=turns {}+".format(
            self.ae_warmup_turns - 1,
            self.ae_warmup_turns,
            self.ae_warmup_turns + self.warmup_B_turns - 1,
            self.ae_warmup_turns + self.warmup_B_turns,
        ))
        ...
        for turn in range(self.train_turn):
            ...
            is_warmup_B = (self.ae_warmup_turns <= turn < self.ae_warmup_turns + self.warmup_B_turns)
            freeze_B    = (turn >= self.ae_warmup_turns + self.warmup_B_turns)
            ...
            for user in select_users:
                ...
                losses.append(loss)
                client_model.append(self.client.upload_model(freeze_B=freeze_B))
            ...
            self.server.aggregation(
                ...
                freeze_B=freeze_B, cdp=self.cdp, ldp=self.ldp
            )
